...
const userBlock = async (body:{userId:ObjectId}) => {
  ...
  const getUsers = async (query:{page?:number, limit?:number, search?:string}) => {
    const {page=1, limit=10, search} = query
    var filter : {isDeleted:boolean, isVerified:boolean,  $or?: Array<{ fName?: { $regex: RegExp } } | { lName?: { $regex: RegExp } } | { email?: { $regex: RegExp } }>} = {
      isDeleted: false,
      isVerified: true,
    };
  ...
  const dashboard = async () => {
    ...
    const [countUser] = await Promise.all([User.countDocuments(userQuery)]);
    return { countUser };
  };
