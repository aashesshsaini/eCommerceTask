...
const jamGet = catchAsync(async (req: Request, res: Response) => {
  ...
  const {
    ...
    attendedJams,
    attendedJamsCount,
  } = await userJamService.jamGet(req.query, req.token.user._id, timeZone);
  return successResponse(
    req,
    res,
    STATUS_CODES.SUCCESS,
...
const jamUpdate = catchAsync(async (req: Request, res: Response) => {
  const jamUpdatedData = await userJamService.jamUpdate(
    req.body,
    req.token.user._id
  );
  return successResponse(
...
const cancelJam = catchAsync(async (req: Request, res: Response) => {
  const jamUpdatedData = await userJamService.cancelJam(
    req.body,
    req.token.user._id
  );
  return successResponse(
...
const getUsers = catchAsync(async (req: Request, res: Response) => {
  const { Users, countUser } = await userJamService.getUsers(
    req.query,
    req.token.user._id
  );
  return successResponse(
...
const favMemberGet = catchAsync(async (req: Request, res: Response) => {
  const { favMemList, favMemCount } = await userJamService.favMemberGet(
    req.query,
    req.token.user._id
  );
  return successResponse(
...
const inviteMembers = catchAsync(async (req: Request, res: Response) => {
  const members = await userJamService.inviteMembers(
    req.body,
    req.token.user._id
  );
  return successResponse(
